import numpy as np
import torch
import utils.lungseg.utils as utils
...
def get_model(modeltype, modelname):
    ...
    if os.path.exists("./lungseg/unet_r231-d5d2fc3d.pth"):
        state_dict = torch.load(
            "./lungseg/unet_r231-d5d2fc3d.pth", map_location=torch.device("cpu")
        )
    else:
...
if __name__ == "__main__":
    image = sitk.ReadImage(
        "/home/winter/Projects/cv/covid19/dataset/Cap subjects/cap001/98C1066-ASAD_100.dcm"
    )
    import pydicom
    import matplotlib.pyplot as plt

    plt.subplot(2, 2, 1)
    plt.imshow(apply(image)[0])
    plt.subplot(2,2,2)
    ...
    sys.path.append("..")
    from transform import ElasticDistortion

    hu = np.stack([hu, hu, hu])
    hu = ElasticDistortion()(hu)
    image = hu[0:1, :]
